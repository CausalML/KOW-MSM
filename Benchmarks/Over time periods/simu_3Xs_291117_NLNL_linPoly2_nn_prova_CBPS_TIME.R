...
for (ttt in seqqtt) {
  ...
  jj <- jj+1
  #print(jj)
  print(paste("Iteration ", jj, "out of ", length(seqqtt), " REP ", ttt ))
  for(ii in 1:ite){
    print(paste("###############################################################################################################################################################", ii))
    ...
    #data <- read.table(file=paste("~/KOW/data/Longdata3X_NL_NL_211117_nn",nn,"_repp",REP,"_",ii,".csv",sep=""), header=T)
    data <- read.table(file=paste("/home/micsan/research/Research/projects/Cornell/Longitudinal/data/Longdata3X_NL_NL_211117_TIME",ttt,"_repp",REP,"_",ii,".csv",sep=""), header=T)
    ...
    form0 <- "Treatment ~ TreatmentIPW + X1 + X2 + X3 + I(X1^2) + I(X2^2) + I(X3^2) + TreatmentIPW:X1 + TreatmentIPW:X2 + TreatmentIPW:X3 + TreatmentIPW:I(X1^2) + TreatmentIPW:I(X2^2) + TreatmentIPW:I(X3^2) + TreatmentIPW:X1:I(X1^2) + TreatmentIPW:X2:I(X2^2) + TreatmentIPW:X3:I(X3^2)"
    ...
    save.image("/home/micsan/research/Research/projects/Cornell/Longitudinal/data/Longdata3X_NL_NL_TIME_2_6_CBPS.RData")
