...
optimize <- function(data){
  ...
  for(t in 1:max(unique(data$time))){
    ...
    if(t==1){
      ...
      temp <- benchmark(gpmlres <-  gpml(data$y[which(data$time==t)],X,1,t),replications=1)$elapsed
      time_gpml_t[t] <- temp
    ...
    if(t>1){
      ...
      temp <- benchmark(gpmlres <- gpml(data$y[which(data$time==t)],Xt,At,t),replications=1)$elapsed
      time_gpml_t[t] <- temp
...
for (ttt in seqqtt) {
  ...
  for(ii in 1:ite){
    ...
    save.image("/home/micsan/research/Research/projects/Cornell/Longitudinal/data/Longdata3X_NL_NL_TIME_2_6.RData") 
