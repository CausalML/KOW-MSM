...
optimize <- function(data,vvv){
  ...
  for(t in 1:max(unique(data$time))){
    ...
    if(t==1){
      ...
      if(vvv==4){
        X <- cbind(data$X1[which(data$time==t)],data$X2[which(data$time==t)],data$X3[which(data$time==t)],data$X4[which(data$time==t)])
      }
      
      if(vvv==5){
        X <- cbind(data$X1[which(data$time==t)],data$X2[which(data$time==t)],data$X3[which(data$time==t)],data$X4[which(data$time==t)],data$X5[which(data$time==t)])
      }
      
      if(vvv==6){
        X <- cbind(data$X1[which(data$time==t)],data$X2[which(data$time==t)],data$X3[which(data$time==t)],data$X4[which(data$time==t)],data$X5[which(data$time==t)],data$X6[which(data$time==t)]) 
      } 
      if(vvv==7){
        X <- cbind(data$X1[which(data$time==t)],data$X2[which(data$time==t)],data$X3[which(data$time==t)],data$X4[which(data$time==t)],
                   data$X5[which(data$time==t)],data$X6[which(data$time==t)],data$X7[which(data$time==t)]) 
      } 
      if(vvv==8){
        X <- cbind(data$X1[which(data$time==t)],data$X2[which(data$time==t)],data$X3[which(data$time==t)],data$X4[which(data$time==t)],
                   data$X5[which(data$time==t)],data$X6[which(data$time==t)],data$X7[which(data$time==t)],data$X8[which(data$time==t)]) 
      } 
  ...
  return(list(result=result,time_gpml=time_gpml_t,time_gurobi=time_gurobi,X=X,Xt=Xt))
...
MA_TIME_TEMP  <- MA_TIME_GPML <- MA_TIME_GUROBI  <- matrix(NA,nrow=ite,ncol=length(seqqVAR))
...
for (vvv in seqqVAR) {
  ...
  for(ii in 1:ite){
    ...
    time_temp[ii] <- benchmark(result_temp <- optimize(data,vvv),replications=1)$elapsed #end system.time
    
    result <- result_temp$result
    time_gpml[ii] <- mean(result_temp$time_gpml)
    time_gurobi[ii] <- result_temp$time_gurobi
    
    save.image("/home/micsan/research/Research/projects/Cornell/Longitudinal/data/Longdata3X_L_L_TIME_REP5_VAR3_8.RData") 
