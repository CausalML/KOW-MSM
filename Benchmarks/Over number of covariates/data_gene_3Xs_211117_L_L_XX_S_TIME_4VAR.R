...
generate <- function(n,repp,tef0,xef1,xef2,xef3,xef4,a,e,b1,b2,b3,b4,f1,f2,f3,f4){
    ...
    L1 <- L2 <- L3 <- L4 <- rep(NA,repp)
    L1IPW <- L2IPW <- L3IPW <- L4IPW <- AIPW <- rep(NA,repp)
    A <- rep(NA,repp)
    L1v <- L1vipw <- NULL
    L2v <- L2vipw <- NULL
    L3v <- L3vipw <- NULL
    ...
    for (i in 1:n){
      ...
      for (j in 2:(repp)){
        ...
        pr    <- (1+exp(-(  a + e*A[j-1] + b1*(L1[j]) + b2*((L2[j])) + b3*(L3[j]) + b4*(L4[j])
    ...
    #I remove the na``
    df_long   <- na.omit(data.frame(df_long, X1 = L1v, X2 = L2v, X3 = L3v, X4 = L4v, TreatmentIPW = Avipw, Treatment = Av))
    ...
    y <- aalp + tef0*aas + xef1*bbs1 + xef2*bbs2 + xef3*bbs3 + .05*bbs1*bbs2 + .05*bbs1*bbs3 + .05*bbs2*bbs3 + .05*bbs1*bbs4 + .05*bbs2*bbs4 + .05*bbs3*bbs4   + rnorm(n,0,5)

    #data <- data.frame(df_long,y,aas,bbs)
    #data <- data.frame(dflong_time,y,aas,bbs1,bbs2,bbs3,wrong_bbs1,wrong_bbs2,wrong_bbs3)
    data <- data.frame(dflong_time,y,aas,bbs1,bbs2,bbs3,bbs4)
    data
}
...
repp <- REP+1
  ...
  for(i in 1:ite){
    ...
    anal.data <- generate(nn,tt,tef0,xef1,xef2,xef3,xef4,a,e,b1,b2,b3,b4,f1,f2,f3,f4)
    #write.table (anal.data, file=paste("~/KOW/data/Longdata3X_L_L_211117_nn",nn,"_repp",REP,"_",i,".csv",sep=""), row.names=F)
    write.table (anal.data, file=paste("/home/micsan/research/Research/projects/Cornell/Longitudinal/data/Longdata3X_L_L_211117_VAR4",tt,"_repp",REP,"_",i,".csv",sep=""), row.names=F)
    #print(i)
