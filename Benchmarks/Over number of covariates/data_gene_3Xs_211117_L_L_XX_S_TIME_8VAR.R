...
generate <- function(n,repp,tef0,xef1,xef2,xef3,xef4,xef5,xef6,xef7,xef8,a,e,b1,b2,b3,b4,b5,b6,b7,b8,f1,f2,f3,f4,f5,f6,f7,f8){
    ...
    L1 <- L2 <- L3 <- L4 <- L5 <- L6 <- L7 <-  L8 <- rep(NA,repp)
    L1IPW <- L2IPW <- L3IPW <- L4IPW <- L5IPW <- L6IPW <-  L7IPW <- L8IPW <- AIPW <- rep(NA,repp)
    A <- rep(NA,repp)
    L1v <- L1vipw <- NULL
    L2v <- L2vipw <- NULL
    L3v <- L3vipw <- NULL
    ...
    for (i in 1:n){
      ...
      for (j in 2:(repp)){
        ...
        pr    <- (1+exp(-(  a + e*A[j-1] + b1*(L1[j]) + b2*((L2[j])) + b3*(L3[j]) + b4*(L4[j]) + b5*(L5[j]) + b6*(L6[j]) + b7*(L7[j]) + b8*(L8[j])
        ...
        if(j<(repp)){ 
          ...
          L5IPW[j+1] <- L5[j]
          L6[j+1]  <- L6[j] + f6*A[j]  + rnorm(1,0,1)
          L6IPW[j+1] <- L6[j]
          L7[j+1]  <- L7[j] + f7*A[j]  + rnorm(1,0,1)
          L7IPW[j+1] <- L7[j]
          L8[j+1]  <- L8[j] + f8*A[j]  + rnorm(1,0,1)
          L8IPW[j+1] <- L8[j]
        }#end if
      ...
      L6vipw <- c(L6vipw,L6IPW)
      L7vipw <- c(L7vipw,L7IPW)
      L8vipw <- c(L8vipw,L8IPW)
      Avipw <- c(Avipw,AIPW)
      Av <- c(Av,A)
    ...
    #I remove the na``
    df_long   <- na.omit(data.frame(df_long, X1 = L1v, X2 = L2v, X3 = L3v, X4 = L4v, X5 = L5v, X6 = L6v, X7 = L7v, X8 = L8v, TreatmentIPW = Avipw, Treatment = Av))
    ...
    aggL6 <- aggregate(df_long$X6,FUN=sum,by=list(df_long$ID))
    aggL7 <- aggregate(df_long$X7,FUN=sum,by=list(df_long$ID))
    aggL8 <- aggregate(df_long$X8,FUN=sum,by=list(df_long$ID))
    ...
    df_long   <- data.frame(df_long,sumL6 = rep(aggL6$x, df$count-1))
    df_long   <- data.frame(df_long,sumL7 = rep(aggL7$x, df$count-1))
    df_long   <- data.frame(df_long,sumL8 = rep(aggL8$x, df$count-1))
    ...
    bbs6 <- dflong_time$sumL6[dflong_time$time==1]
    bbs7 <- dflong_time$sumL7[dflong_time$time==1]
    bbs8 <- dflong_time$sumL8[dflong_time$time==1]
    ...
    - .05*mean(bbs1*bbs5)  - .05*mean(bbs2*bbs5) - .05*mean(bbs3*bbs5) - .05*mean(bbs4*bbs5)  - .05*mean(bbs1*bbs6)  - .05*mean(bbs2*bbs6) - .05*mean(bbs3*bbs6) - .05*mean(bbs4*bbs6) - .05*mean(bbs5*bbs6)
    
    y <- aalp + tef0*aas + xef1*bbs1 + xef2*bbs2 + xef3*bbs3 + .05*bbs1*bbs2 + .05*bbs1*bbs3 + .05*bbs2*bbs3 + .05*bbs1*bbs4 + .05*bbs2*bbs4 + .05*bbs3*bbs4 + .05*bbs1*bbs5 + .05*bbs2*bbs5 + .05*bbs3*bbs5 + .05*bbs4*bbs5 
    + .05*bbs1*bbs6 + .05*bbs2*bbs6 + .05*bbs3*bbs6 + .05*bbs4*bbs6 + .05*bbs5*bbs6 + .05*bbs1*bbs7 + .05*bbs2*bbs7 + .05*bbs3*bbs7 + .05*bbs4*bbs7 + .05*bbs5*bbs7 + .05*bbs6*bbs7
    + .05*bbs1*bbs8 + .05*bbs2*bbs8 + .05*bbs3*bbs8 + .05*bbs4*bbs8 + .05*bbs5*bbs8 + .05*bbs6*bbs8+ .05*bbs7*bbs8 + rnorm(n,0,5)

    #data <- data.frame(df_long,y,aas,bbs)
    #data <- data.frame(dflong_time,y,aas,bbs1,bbs2,bbs3,wrong_bbs1,wrong_bbs2,wrong_bbs3)
    data <- data.frame(dflong_time,y,aas,bbs1,bbs2,bbs3,bbs4,bbs5,bbs6,bbs7,bbs7)
    data
}
...
repp <- REP+1
  ...
  for(i in 1:ite){
    ...
    anal.data <- generate(nn,tt,tef0,xef1,xef2,xef3,xef4,xef5,xef6,xef7,xef8,a,e,b1,b2,b3,b4,b5,b6,b7,b8,f1,f2,f3,f4,f5,f6,f7,f8)
    #write.table (anal.data, file=paste("~/KOW/data/Longdata3X_L_L_211117_nn",nn,"_repp",REP,"_",i,".csv",sep=""), row.names=F)
    write.table (anal.data, file=paste("/home/micsan/research/Research/projects/Cornell/Longitudinal/data/Longdata3X_L_L_211117_VAR8",tt,"_repp",REP,"_",i,".csv",sep=""), row.names=F)
    #print(i)
