...
generate <- function(n,repp,tef0,xef1,xef2,xef3,xef4,xef5,xef6,xef7,a,e,b1,b2,b3,b4,b5,b6,b7,f1,f2,f3,f4,f5,f6,f7){
    ...
    L1 <- L2 <- L3 <- L4 <- L5 <- L6 <- L7 <- rep(NA,repp)
    L1IPW <- L2IPW <- L3IPW <- L4IPW <- L5IPW <- L6IPW <-  L7IPW <- AIPW <- rep(NA,repp)
    A <- rep(NA,repp)
    L1v <- L1vipw <- NULL
    L2v <- L2vipw <- NULL
    L3v <- L3vipw <- NULL
    ...
    for (i in 1:n){
      ...
      for (j in 2:(repp)){
        ...
        pr    <- (1+exp(-(  a + e*A[j-1] + b1*(L1[j]) + b2*((L2[j])) + b3*(L3[j]) + b4*(L4[j]) + b5*(L5[j]) + b6*(L6[j]) + b7*(L7[j])
    ...
    #I remove the na``
    df_long   <- na.omit(data.frame(df_long, X1 = L1v, X2 = L2v, X3 = L3v, X4 = L4v, X5 = L5v, X6 = L6v, X7 = L7v, TreatmentIPW = Avipw, Treatment = Av))
    ...
    + .05*bbs1*bbs6 + .05*bbs2*bbs6 + .05*bbs3*bbs6 + .05*bbs4*bbs6 + .05*bbs5*bbs6 + .05*bbs1*bbs7 + .05*bbs2*bbs7 + .05*bbs3*bbs7 + .05*bbs4*bbs7 + .05*bbs5*bbs7 + .05*bbs6*bbs7 + rnorm(n,0,5)

    #data <- data.frame(df_long,y,aas,bbs)
    #data <- data.frame(dflong_time,y,aas,bbs1,bbs2,bbs3,wrong_bbs1,wrong_bbs2,wrong_bbs3)
    data <- data.frame(dflong_time,y,aas,bbs1,bbs2,bbs3,bbs4,bbs5,bbs6,bbs7)
    data
}
