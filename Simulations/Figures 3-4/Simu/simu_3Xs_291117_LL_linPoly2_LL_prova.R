...
KGram <- function(XX,AA,t,scale1,scale){
  ...
  if(t==1){
    spl <- polydot(degree = 2, scale = 0.27, offset = 1)
  ...
  if(t>1){
    ...
    spl <- polydot(degree = 2, scale = 0.3, offset = 1)
...
for (ll in seqqL) {
  ...
  for(ii in 1:ite){
    ...
    data <- read.table(file=paste("~/KOW/data/Longdata3X_L_L_211117_nn",n,"_repp",REP,"_",ii,".csv",sep=""), header=T)
    ...
    if (class(result) != "try-error") {
      ...
      save.image("~/KOW/data/simu_141217_LL_linpoly2_LL.RData")
