...
source("~/KOW/Rcode/OverL/calc_ipw2.R")
...
KGram <- function(XX,AA,t,scale1,scale){
  ...
  if(t==1){
    spl <- polydot(degree = 2, scale = 3.22, offset = 1)
  ...
  if(t>1){
    ...
    spl <- polydot(degree = 2, scale = 1.37, offset = 1)
...
seqqL <- c(seq(0,100,length.out=25),seq(101,1000,length.out = 25)) 
...
for (ll in seqqL) {
  ...
  jj <- jj+1
  #print(jj)
  print(paste("Iteration ", jj, "out of ", length(seqqL)))
  for(ii in 1:ite){
    #print(paste("###############################################################################################################################################################", ii))
    ...
    #data <- read.table(file=paste("/home/micsan/research/Research/projects/Cornell/Longitudinal/data/Longdata3X_L_L_211117_nn",n,"_repp",REP,"_",ii,".csv",sep=""), header=T)
    data <- read.table(file=paste("~/KOW/data/Longdata3X_NL_NL_211117_nn",n,"_repp",REP,"_",ii,".csv",sep=""), header=T)
    ...
    ipw_1 <- calc_ipw_1(data)$ipw_1
    sipw_1 <- calc_ipw_1(data)$sipw_1

    ipw_2 <- calc_ipw_2(data)$ipw_2
    sipw_2 <- calc_ipw_2(data)$sipw_2
    ...
    for(t in 1:max(unique(data$time))){
      ...
      if(t==1){
        ...
        K <- KGram(X,1,t,scale1=1,scale=1)
    ...
    if (class(result) != "try-error") {
      ...
      MM[ii]<-coef(lm(y~aas))[2]
      MW2[ii]<-coef(lm(y~aas,w=W))[2]
      MIPW[ii]<-coef(lm(y~aas,w=ipw_1))[2]
      MSIPW[ii]<-coef(lm(y~aas,w=sipw_1))[2]
      
      MIPW_2[ii]<-coef(lm(y~aas,w=ipw_2))[2]
      MSIPW_2[ii]<-coef(lm(y~aas,w=sipw_2))[2]
      
      BM[ii]<-MM[ii]-delta
      B[ii]<-MW2[ii]-delta
      BIPW[ii]<-MIPW[ii]-delta
      BSIPW[ii]<-MSIPW[ii]-delta
      
      BIPW_2[ii]<-MIPW_2[ii]-delta
      BSIPW_2[ii]<-MSIPW_2[ii]-delta
      
      SDM[ii]<-sqrt(diag(sandwich(lm(y~aas))))[2]
      SDW[ii]<-sqrt(diag(sandwich(lm(y~aas,w=W))))[2]
      SDIPW[ii]<-sqrt(diag(sandwich(lm(y~aas,w=ipw_1))))[2]
      SDSIPW[ii]<-sqrt(diag(sandwich(lm(y~aas,w=sipw_1))))[2]
      SDIPW_2[ii]<-sqrt(diag(sandwich(lm(y~aas,w=ipw_2))))[2]
      SDSIPW_2[ii]<-sqrt(diag(sandwich(lm(y~aas,w=sipw_2))))[2]
      
      #95% COVERAGE
      
      COVM[ii] <- (delta>(MM[ii]-qnorm(.975)*SDM[ii]))*(delta<(MM[ii]+qnorm(.975)*SDM[ii]))
      COVW[ii] <- (delta>(MW2[ii]-qnorm(.975)*SDW[ii]))*(delta<(MW2[ii]+qnorm(.975)*SDW[ii]))
      COVIPW[ii] <- (delta>(MIPW[ii]-qnorm(.975)*SDIPW[ii]))*(delta<(MIPW[ii]+qnorm(.975)*SDIPW[ii]))
      COVSIPW[ii] <- (delta>(MSIPW[ii]-qnorm(.975)*SDSIPW[ii]))*(delta<(MSIPW[ii]+qnorm(.975)*SDSIPW[ii]))
      COVIPW_2[ii] <- (delta>(MIPW_2[ii]-qnorm(.975)*SDIPW_2[ii]))*(delta<(MIPW_2[ii]+qnorm(.975)*SDIPW_2[ii]))
      COVSIPW_2[ii] <- (delta>(MSIPW_2[ii]-qnorm(.975)*SDSIPW_2[ii]))*(delta<(MSIPW_2[ii]+qnorm(.975)*SDSIPW_2[ii]))
      
      ER[ii]<-(MM[ii]-delta)^2
      ERR[ii]<-(MW2[ii]-delta)^2
      ERRIPW[ii]<-(MIPW[ii]-delta)^2
      ERRSIPW[ii]<-(MSIPW[ii]-delta)^2
      
      ERRIPW_2[ii]<-(MIPW_2[ii]-delta)^2
      ERRSIPW_2[ii]<-(MSIPW_2[ii]-delta)^2
      
      
      
      save.image("~/KOW/data/simu_141217_NLNL_linpoly2_NLNL.RData")
      
    }  
  }  
  
  MMM[jj] <- mean(MM,na.rm=T)
  MMW2[jj] <- mean(MW2,na.rm=T)    
  MMIPW[jj] <- mean(MIPW,na.rm=T)
  MMSIPW[jj] <- mean(MSIPW,na.rm=T)
  
  MMIPW_2[jj] <- mean(MIPW_2,na.rm=T)
  MMSIPW_2[jj] <- mean(MSIPW_2,na.rm=T)
  
  MCOVM[jj] <- mean(COVM,na.rm = T)
  MCOVW[jj] <- mean(COVW,na.rm = T)
  MCOVIPW[jj] <- mean(COVIPW,na.rm = T)
  MCOVSIPW[jj] <- mean(COVSIPW,na.rm = T)
  MCOVIPW_2[jj] <- mean(COVIPW_2,na.rm = T)
  MCOVSIPW_2[jj] <- mean(COVSIPW_2,na.rm = T)
  
  MBM[jj] <- mean(BM,na.rm=T)
  MB[jj] <- mean(B,na.rm=T)
  MBIPW[jj] <- mean(BIPW,na.rm=T)
  MBSIPW[jj] <- mean(BSIPW,na.rm=T)
  
  MBIPW_2[jj] <- mean(BIPW_2,na.rm=T)
  MBSIPW_2[jj] <- mean(BSIPW_2,na.rm=T)
  
  
  MSEM[jj] <- sd(MM,na.rm=T)
  MSEMW[jj] <- sd(MW2,na.rm=T)
  MSEIPW[jj] <- sd(MIPW,na.rm=T)
  MSESIPW[jj] <- sd(MSIPW,na.rm=T)
  
  MSEIPW_2[jj] <- sd(MIPW_2,na.rm=T)
  MSESIPW_2[jj] <- sd(MSIPW_2,na.rm=T)
  
  # mean(SDW)    
  # mean(SDIPW)
  
  MER[jj] <- mean(ER,na.rm=T)
  MERR[jj] <- mean(ERR,na.rm=T)    
  MERRIPW[jj] <- mean(ERRIPW,na.rm=T) 
  MERRSIPW[jj] <- mean(ERRSIPW,na.rm=T) 
  
  MERRIPW_2[jj] <- mean(ERRIPW_2,na.rm=T) 
  MERRSIPW_2[jj] <- mean(ERRSIPW_2,na.rm=T) 
  
  MSCALE[jj] <- mean(scale_ll,na.rm=T)
  MSCALE2[jj] <- mean(scale_ll2,na.rm=T)
  
  MLA1[jj] <- mean(LAMB1,na.rm=T)
  MLA2[jj] <- mean(LAMB2,na.rm=T)
